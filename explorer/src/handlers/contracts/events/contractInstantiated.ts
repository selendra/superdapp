...
export const contractsInstantiatedHandler: EventHandler = {
  ...
  handle: async (
    ...
    if (extrinsic && call) {
      ...
      const { deployer, contract } = new api.events.NormalisedContractsInstantiatedEvent(
        ctx,
        event
      ).resolve()
      ...
      const contractInfo = await new api.storage.NormalisedContractInfoOfStorage(
        ctx,
        block
      ).get(contract)
