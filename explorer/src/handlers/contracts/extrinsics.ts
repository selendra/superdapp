...
import { Ctx, ExtrinsicHandler, OptEntity } from '../../interfaces/handler'
import {
  createActivity,
  createExtrinsic,
  getOrCreateAccount,
...
export const contractsCallHandler: ExtrinsicHandler = {
  name: 'Contracts.call',
  handle: async (
    ctx: Ctx,
    call: SubstrateCall,
    ...
    const { store, log } = ctx
    const { contractAddress, data } = new api.calls.NormalisedContractsCallCall(
      ctx,
      call
    ).resolve()
